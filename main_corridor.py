...
if __name__ == '__main__':
    ...
    Arterial_pdf = 'Sample Arterial Green Report_NW 103 St (4).pdf'
    ...
    travel_time_xml_file = 'Corridor Travel time NW 103 St (3).xml'
    ...
    renamed_travel_time_xml_file = 'Travel_Time_Corridor.xml'
    shutil.copyfile(travel_time_xml_file, renamed_travel_time_xml_file)
    travel_time_excel_file = 'Travel_Time_Corridor'  # suffix will be added by function SaveAs
    ...
    # Fill out columns: NT, ST columns name: 'C' 'F'
    # TODO this column is involving other two pdf files
    Arterial_green_time = pdf_to_csv(Arterial_pdf, separator, threshold)
    Arterial_green_time_lines = Arterial_green_time.split('\n')
    direction_index = 0
    # The last three rows are about page number, they are useless
    first_loop = True
    for Arterial_green_time_lines_data_zone in Arterial_green_time_lines[10:-3]:
        if ('WB' in Arterial_green_time_lines_data_zone.split('|')[3] and
            'EB' in Arterial_green_time_lines_data_zone.split('|')[3]):
            if first_loop:
                min_first_avg_green = Arterial_green_time_lines_data_zone.split('|')[8]
                min_second_ave_green = Arterial_green_time_lines_data_zone.split('|')[12]
            elif min_first_avg_green > Arterial_green_time_lines_data_zone.split('|')[8]:
                min_first_avg_green = Arterial_green_time_lines_data_zone.split('|')[8]
            elif min_second_ave_green > Arterial_green_time_lines_data_zone.split('|')[12]:
                min_second_ave_green = Arterial_green_time_lines_data_zone.split('|')[12]
        first_loop = False
    # print min_first_avg_green
    # print min_second_ave_green
    total_lines = 1440  # TODO this total line number should be calculated based on users input
    for line in range(2, total_lines + 2):
        sheet['C' + str(line)] = min_first_avg_green
        sheet['F' + str(line)] = min_second_ave_green

    # Fill out column Time Interval (NT), Time Interval (ST),
    # columns name 'D' 'G'
    ...
    book.save('bluetooth_V2_corridor_output.xlsx')
